...
\begin{tikzpicture}[remember picture,overlay]
    ...
    \node[minimum height = 2.3cm,minimum width=8cm,anchor=south west,font=\color{white}\Huge\linespread{1}\selectfont,rectangle,yshift=-4.8cm,xshift=2.1cm,align=left] at (current page.north west){\textsc{BEE HABITAT}\\\textsc{\huge{STEWARDSHIP TOOL}}};
    ...
    \node[anchor=south east,rectangle,,yshift=0.2cm,xshift=0cm] at (current page marginpar area.north east){\includegraphics[width=3.3cm]{../extdata/OregonFlora_logo.jpg}};
...
\section{Bee habitat from your iNaturalist project}
...
Based on bee-plant records from Oregon Bee Atlas, we predict that you have \Large`r nrow(suppBeeSpp)`\footnote{Similar sites in Oregon range from `r paste0(range(sapply(iNatNetworks,function(x) length(unique(strsplit(paste0(x$genList$beeSpp,collapse=','),',')[[1]])))),collapse=' - ')` species of bees} \Large species of bees across `r famCounts %>% filter(cat=='existing') %>% pull(lookupFam) %>% unique %>% length` families. `r ifelse(isMultiyear,"You provided us with multiple years of data, so any year-to-year improvements in bee habitat from additional plant species you discovered in each year are shown below.","")`
