...
# params <- list(set_title = "callotchrissy")
...
vinePlDat_year <- iNatPlDat_vy %>% st_drop_geometry() %>% 
...
# vinePlDatSpp_year <- iNatPlDat_vy %>% st_drop_geometry() %>% #Same as above, but for full scientific name
...
# dat_vinePlant <- beeData_ecoreg %>% filter(plantGenus %in% unique(iNatPlDat_vy$plGenus))
# datSpp_vinePlant <- beeData_ecoreg %>% filter(foragePlant %in% unique(iNatPlDat_vy$scientific_name)) #Same as above, but for full scientific name
...
vineGenMat <- ecoRegNetworks[[ecoRegName]]$ntwk_gen_all[rownames(ecoRegNetworks[[ecoRegName]]$ntwk_gen_all) %in% unique(iNatPlDat_vy$plGenus),] 
...
vinePlDat_spp <- iNatPlDat_vy %>% st_drop_geometry() %>% 
...
vinePlDat_gen <- iNatPlDat_vy %>% st_drop_geometry() %>% 
...
\begin{multicols}{2}[
Out of the `r nrow(vinePlDat_spp)` identified plant species from your iNaturalist project, \Large`r nrow(filter(vinePlDat_spp,!is.na(N)&quality!='poor'))` \Large are bee-friendly plants. Of these plants:
]
...
\item \huge`r nrow(filter(vinePlDat_spp,!is.na(N)&quality!='poor'&isNative))` \Large species are native to your Ecoregion
\item \huge`r nrow(filter(vinePlDat_spp,!is.na(N)&quality!='poor'&isWeedy))` \Large species are not native to your Ecoregion
