...
if(nrow(vyCombos_vy)==0){ 
  ...
  vyCombos_vy %>% rename_with(.fn=~gsub('Common_Name','CommonName',.x),.cols = everything()) %>% 
    ...
    mutate(CommonName=gsub(',.*','',str_to_title(CommonName))) %>% #Gets rid of multiple names
    mutate(across(c(CommonName,Abstract), ~gsub("('|’)","\\\\textquotesingle ",.x))) %>% #Escapes 2 types of apostrophes 
    arrange(desc(partner)) %>% 
    mutate(Species=paste0("\\textbf{",CommonName,"}"," (\\emph{",ScientificName,"})")) %>% 
    ...
    column_spec(1,width='1.5cm') %>% column_spec(2,width='4.5cm') %>% column_spec(3,width='10cm')
}
