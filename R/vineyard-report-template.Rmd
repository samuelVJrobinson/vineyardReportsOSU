...
if(nrow(superPl_spp)>0){
  ...
  kable(escape=FALSE,format = 'latex',caption = 'Super bee plant species', position = "h") %>% 
...
if(nrow(superPl_gen)>0){
  ...
  superPl_gen %>%
    ...
    transmute(Plant=linebreak(paste0(Common_name,'\n(\\emph{',plGenus,'})'),align='r'),
              Origin=ifelse(isNative,'Native','Introduced'),`Life cycle`=Lifecycle,
              `Bloom Time`=paste0(Bloom_start,'-',Bloom_end)) %>% 
    kable(escape=FALSE,format = 'latex',caption = 'Super bee plant genera', position = "h") %>%  
    column_spec(3,width='3cm') 
...
```{r fieldMapsSpp, out.width = '100%', fig.asp=0.75, warning=FALSE}
...
```{r fieldMapsGen, out.width = '100%', fig.asp=0.75, warning=FALSE}
# p2 <- 
ggplot() + geom_stars(data=st_rgb(b))+
...
data.frame(genSpp=plantScoresSpp$PlantGenus) %>% 
  ...
  add_header_above(c(" "=4,"Planting Type"=5),line = TRUE)
...
`r if(vyCombos_vy$plant=='All weeds') print('All plants listed in iNaturalist project were weedy. See pg. 4 for future planting suggestions')` 

`r if(nrow(vyCombos_vy)!=0 & vyCombos_vy$plant!='All weeds') print("Your property had the following plant species, which is visited by the following unique unique bee:")`
...
suppBeeSpp %>% select(-contains('in')) %>% rename(Family=lookupFam) %>% 
  ...
  column_spec(2,width='12cm',italic = TRUE)
