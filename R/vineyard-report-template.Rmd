...
\begin{tikzpicture}[remember picture,overlay]
    ...
    \node[anchor=north west,rectangle,,yshift=-0.5cm,xshift=0.5cm] at (current page.north west){\includegraphics[height=2.3cm]{../data/images/OSU_ES-MM_horizontal_1C_W.png}};
    ...
    \node[anchor=south east,rectangle,,yshift=0cm,xshift=-3cm] at (current page marginpar area.north east){\includegraphics[height=5cm]{../data/images/Grumpbee transparent.png}};
    \node[anchor=south east,rectangle,,yshift=0cm,xshift=0cm] at (current page marginpar area.north east){\includegraphics[height=2.3cm]{../data/images/OregonFlora logo.jpg}};
...
\includegraphics[width=2.5cm]{../data/images/purple-fireweed-hi.png}
...
```{r fieldMaps, out.width = '100%', fig.asp=0.75, warning=FALSE}
...
p1 <- ggplot() + geom_stars(data=st_rgb(b))+
...
p2 <- ggplot() + geom_stars(data=st_rgb(b))+
...
data.frame(genSpp=plantScoresSpp$PlantGenus) %>% 
  ...
  group_by(Lifecycle) %>% slice(1:nPlantsMax) %>% ungroup() %>% 
  kable(escape=FALSE,format = 'latex',longtable=TRUE,align = c(rep('r', 2),rep('l', 3),rep('c',6))) %>% 
  kable_styling(latex_options = c("repeat_header","scale_down")) %>% 
  column_spec(1,width='0.75cm') %>% 
  column_spec(3:5,width='2cm',latex_valign = 'm') %>% #Warning: arguement not atomic vector
...
if(nrow(vyCombos_vy)==0){
  ...
  vyCombos_vy %>% rename_with(.fn=~gsub('Common_Name','CommonName',.x),.cols = everything()) %>% 
    ...
    column_spec(4,width='10cm') %>% kable_styling(latex_options="scale_down")
}
...
suppBeeSpp %>% select(-contains('in')) %>% rename(`Bee Species` = genSpp) %>% 
  kable()
...
data.frame(genSpp=plantScoresSpp$PlantGenus) %>% 
  ...
  kable_styling(latex_options = c("repeat_header","scale_down")) %>% 
  column_spec(1,width='0.75cm') %>% 
  column_spec(3:5,width='2cm',latex_valign = 'm') %>% #Warning: argument not atomic vector
  column_spec(6:11,latex_valign = 'm') %>% 
  add_header_above(c(" "=5,"Planting Type"=6),line = TRUE)
```
