...
output: 
  bookdown::pdf_document2:
    ...
    keep_tex: false
    includes:
      in_header: ./data/vineyard-report-header.tex
params:
  set_title: "NULL"
  wd_path: "NULL"
geometry: margin=2cm
...
vinePlDat_vy <- filter(vinePlDat, vineyard==paramList$set_title) 
...
# plantScoresGen <- getPlGenScores(sppMat,unique(vinePlDat_vy$plGenus),exclude = nonNativeGen) %>% 
#   select(-Chao1,-Chao1Z) %>% 
...
plSppRich <- ecoRegNetworks[[ecoRegName]]$topSpp %>% 
  full_join(plantList,by=c('plantSpp'='Scientific_name')) %>% 
  ...
  mutate(across(c(N,EstRich,Nrare),~ifelse(is.na(.x),0,.x)),
         quality=ifelse(is.na(quality),'poor',quality)) #Fills in unmatched records with zeros (not visited)
...
`r paste0(paramList$set_title,': REPORT 2025')`
