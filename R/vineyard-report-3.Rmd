...
output: 
  bookdown::pdf_document2:
    ...
    keep_tex: false
    includes:
      in_header: vineyard-report-2-headIncl.tex
params:
  set_title: "Abbott_Claim"
...
# params: 
#   set_title: "tempTitle" #Include as formal parameters once
#   csv_path: "temp.csv" 
...
library(vegan)
library(bipartite)
# library(ggmap) #Requires Google API Key
library(basemaps) #https://jakob.schwalb-willmann.de/basemaps/ #Try this instead
library(stars)
...
#                wd_path = "./reports/")
oldWd <- getwd()
# setwd(params$wd_path)

load(params$data_path) #Load OBA bee data

#Load vineyard plant data from iNaturalist records
vinePlDat_all <- vinePlDat #All vineyard data

vinePlDat <- vinePlDat %>% #Plants from focal vineyard
  filter(vineyard==params$set_title) 

if(length(unique(vinePlDat$ecoreg))!=1){
  stop(paste0('Number of ecoregions (',length(unique(vinePlDat$ecoreg)),') for this vineyard is incorrect: should be 1'))
} else {
  ecoRegName <- unique(vinePlDat$ecoreg)
}

isMultiyear <- length(unique(vinePlDat$year))>1 #More than 1 year of data?

#All Oregon bee data - 75,895 rows
beeDataAll <- beeData
#Ecoregion-specific bee data
beeData <- beeData %>% filter(ecoreg==ecoRegName) %>% 
  st_drop_geometry() %>% 
  select(-recordedBy,-sex,-samplingProtocol,-ecoreg)

#Plant genera found in vineyard during different years
vinePlDat_year <- vinePlDat %>% st_drop_geometry() %>% 
  select(plGenus,year) %>% distinct() %>% count(plGenus,year) %>% 
  pivot_wider(names_from=year,values_from = 'n',names_prefix = 'in',
              values_fill = 0,values_fn = ~.x==1)
# vinePlDatSpp_year <- vinePlDat %>% st_drop_geometry() %>% #Same as above, but for full scientific name
...
# dat_vinePlant <- beeData %>% filter(plantGenus %in% unique(vinePlDat$plGenus))
# datSpp_vinePlant <- beeData %>% filter(foragePlant %in% unique(vinePlDat$scientific_name)) #Same as above, but for full scientific name
dat_vinePlant <- beeData %>% st_drop_geometry() %>% 
...
# plantScoresGen <- getPlGenScores(sppMat,unique(vinePlDat$plGenus),exclude = nonNativeGen) %>% 
#   select(-Chao1,-Chao1Z) %>% 
#   filter(PlantGenus %in% plantList$Scientific_name) 

plantScoresSpp <- getPlGenScores(mat = ecoRegNetworks[[ecoRegName]]$ntwk_all, #Regional network (plant spp - bee spp)
                               plGen = unique(vinePlDat$scientific_name[!grepl('spp.',vinePlDat$scientific_name)]), #Plants from focal vineyard
...
vineGenMat <- ecoRegNetworks[[ecoRegName]]$ntwk_gen_all[rownames(ecoRegNetworks[[ecoRegName]]$ntwk_gen_all) %in% unique(vinePlDat$plGenus),] 

#Vineyard plant genera that have no visiting bees (using data from the regional network)
badPlGen <- sort(unique(vinePlDat$plGenus)[!unique(vinePlDat$plGenus) %in% unique(beeData$plantGenus)])
...
suppBeeGen <- dat_vinePlant %>% filter(!is.na(genus)) %>% select(genus,contains('in20')) %>% distinct() %>% #Bee genera 
  group_by(genus) %>% summarize(across(contains('in20'),~any(.x)))
suppBeeSpp <- dat_vinePlant %>% filter(!is.na(species)) %>% select(genSpp,contains('in20')) %>% distinct() %>% #Bee species
  group_by(genSpp) %>% summarize(across(contains('in20'),~any(.x)))
#Regional bee diversity ("gamma diversity")
regBeeGen <- beeData %>% filter(!is.na(species)) %>% select(genus) %>% distinct() #Bee genera
regBeeSpp <- beeData %>% filter(!is.na(species)) %>% select(genSpp) %>% distinct() #Bee species

#Counts of supported bees by family (existing and suggested 5 new plants)
famCounts <- bind_rows(mutate(suppBeeSpp,cat='existing'),
                       data.frame(genSpp=trimws(strsplit(paste0(plantScoresSpp$newSpp[1:5],collapse=', '),split = ',')[[1]]),in2022=FALSE,in2023=FALSE,cat='new')) %>% 
...
plSppRich <- ecoRegNetworks[[ecoRegName]]$topSpp %>% 
  left_join(plantList,by=c('plantSpp'='Scientific_name')) %>% 
  transmute(genSpp=plantSpp,N=Nbees,EstRich=Nbees_estim,Nrare=Nbees_rare,
...
plGenRich <- ecoRegNetworks[[ecoRegName]]$topGen %>% mutate(plantGen = paste0(plantGen,' spp.')) %>% 
  left_join(filter(plantList,grepl('spp.',Scientific_name)),by=c('plantGen'='Scientific_name')) %>% 
...
vinePlDat_spp <- vinePlDat %>% st_drop_geometry() %>% 
  filter(!grepl('.spp',scientific_name)) %>% #Removes genus only records
  count(scientific_name,name = 'Nplants') %>% #N = Number of total plants recorded
  left_join(plSppRich,c('scientific_name'='genSpp'))

vinePlDat_gen <- vinePlDat %>% st_drop_geometry() %>% 
  count(plGenus,name = 'Nplants') %>% 
...
\begin{tikzpicture}[remember picture,overlay]
    ...
    \node[anchor=north west,rectangle,,yshift=-0.5cm,xshift=0.5cm] at (current page.north west){\includegraphics[height=2.3cm]{./images/OSU_ES-MM_horizontal_1C_W.png}};
    ...
    \node[anchor=south east,rectangle,,yshift=0cm,xshift=-3cm] at (current page marginpar area.north east){\includegraphics[height=5cm]{./images/Grumpbee transparent.png}};
    \node[anchor=south east,rectangle,,yshift=0cm,xshift=0cm] at (current page marginpar area.north east){\includegraphics[height=2.3cm]{./images/OregonFlora logo.jpg}};
...
\includegraphics[width=2.5cm]{./images/purple-fireweed-hi.png}
...
if(isMultiyear){
  ...
  coord_flip()+
  scale_fill_manual(values = rev(c('white','grey30','black')))
...
A *super bee plant* attracts a wide diversity of bees, rare specialist bees, and is also not a noxious weed. Some weeds are included in this list, because they are contributing to your bee diversity.
...
```{r superBeePlants}
numPlants <- 8 #How many plants to display?
...
if(nrow(superPl_spp)>0){
  ...
  transmute(Plant=linebreak(paste0(Common_name,'\n(\\emph{',scientific_name,'})'),align='r'),
            Origin=ifelse(isNative,'Native','Introduced'),`Life cycle`=Lifecycle,
            `Bloom Time`=paste0(Bloom_start,'-',Bloom_end)) %>%
  kable(escape=FALSE,format = 'latex',caption = 'Super bee plant species') %>% 
...
if(nrow(superPl_gen)>0){
  ...
  superPl_gen %>%
    ...
    transmute(Plant=linebreak(paste0(Common_name,'\n(\\emph{',plGenus,'})'),align='r'),
              Origin=ifelse(isNative,'Native','Introduced'),`Life cycle`=Lifecycle,
              `Bloom Time`=paste0(Bloom_start,'-',Bloom_end)) %>% 
    kable(escape=FALSE,format = 'latex',caption = 'Super bee plant genera') %>% 
    column_spec(3,width='3cm') 
  
}

if(nrow(superPl_spp)==0 & nrow(superPl_gen)==0){
  print('No super bee plants found. See page 4 for plant species suggestions.')
}
...
```{r fieldMaps, out.width = '100%', fig.asp=1}

plantLocs <- vinePlDat %>% select(-vineyard,-ecoreg) %>% 
...
ext <- st_as_sfc(st_bbox(plantLocs), crs=st_crs(plantLocs)) 
b <- basemap_stars(ext, map_service = "esri", map_type = "world_imagery",verbose = FALSE)

ggplot() + geom_stars(data=st_rgb(b))+
  geom_sf(data=st_transform(plantLocs,st_crs(b)),aes(col=quality.x,shape=taxonLevel))+
  # geom_text_rep(data=filter(st_transform(plantLocs,st_crs(b)),quality.x=='super'),aes(label=plGenus),col='cyan')+
...
data.frame(genSpp=plantScoresSpp$PlantGenus) %>% 
  ...
  mutate(Rank=1:n(),Plant=linebreak(paste0(Common_name,'\n(\\emph{',genSpp,'})'),align='r'),
            Origin=ifelse(isNative,'Native','Introduced'),
  ...
  column_spec(3:5,width='2cm',latex_valign = 'm') %>% 
  column_spec(6:11,latex_valign = 'm') %>% 
  add_header_above(c(" "=5,"Planting Type"=6),line = TRUE)
...
```{r uniqueBeesPlants}
vyCombos <- vyCombos %>% filter(vineyard==params$set_title) %>% select(-vineyard)

if(nrow(vyCombos)==0){
  stop('Vineyard not found in list of unique bees and plants.')
} else if(vyCombos$plant=='All weeds'){
  print('All plants listed in iNaturalist project were weedy. See pg. 4 for future planting suggestions')
} else {
  vyCombos %>% rename_with(.fn=~gsub('Common_Name','CommonName',.x),.cols = everything()) %>% 
    ...
    relocate(partner,CommonName,ScientificName,Abstract) %>% 
    mutate(CommonName=paste0('\\textbf{',CommonName,'}')) %>% 
    mutate(ScientificName=paste0('\\emph{',ScientificName,'}')) %>% 
    kable(escape=FALSE,format = 'latex',col.names = c('Partner','Common Name','Scientific Name','Description')) %>% 
